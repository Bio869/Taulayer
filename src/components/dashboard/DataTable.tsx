...
const generateMockData = (): DataRow[] => {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const minCost = Math.min(...suggestions.map(s => s.estimated_new_cpr_usd));
    const minLatency = Math.min(...suggestions.map(s => s.estimated_new_latency_ms));
    
    const costSaved = Math.max(0, originalCost - minCost);
    const timeSaved = Math.max(0, originalLatency - minLatency);
    ...
    mockRows.push({
      ...
      pct_time_saved: (timeSaved / originalLatency) * 100,
      total_cost_saved_usd: costSaved,
      pct_cost_saved: (costSaved / originalCost) * 100
    });
  }
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Top 3 Suggestions</TableHead>
                ...
                paginatedData.map((row, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      ...
                      >
                        <span>{row.suggestions.length} suggestions</span>
                    ...
                    <TableCell>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          Original latency - min(suggested latencies)
                        </TooltipContent>
                      </Tooltip>
                    </TableCell>
                    <TableCell>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          (Time saved / Original latency) × 100
                        </TooltipContent>
                      </Tooltip>
                    </TableCell>
                    <TableCell>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          Original cost - min(suggested costs)
                        </TooltipContent>
                      </Tooltip>
                    </TableCell>
                    <TableCell>
